...
rt3d::materialStruct material2 = {
	...
	{ 0.0f, 1.0f, 1.0f, 1.0f }, // diffuse
	{ 0.0f, 1.0f, 1.0f, 1.0f }, // specular
	10.0f  // shininess
};
...
void init(void) {
	...
	reflectionMapProgram = rt3d::initShaders("reflectionMap.vert", "reflectionMap.frag");


	texturedProgram = rt3d::initShaders("textured.vert", "textured.frag");
...
void draw(SDL_Window * window) {
	...
	// draw a cube blocks on top of ground plane...
	glUseProgram(toonShaderProgram);
	glBindTexture(GL_TEXTURE_2D, textures[0]);
	for (int b = 0; b <5; b++)
	{
		mvStack.push(mvStack.top());
			mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-20.0f + b * 5, 1.0f + b, -16.0f));
	...
	GLuint uniformIndex = glGetUniformLocation(reflectionMapProgram, "cubeMap");
	...
	modelMatrix = glm::rotate(modelMatrix, float(float(theta)*DEG_TO_RADIAN), glm::vec3(1.0f, 1.0f, 1.0f));//transform theta to radians if expressed in degrees
	...
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-20.0f, 1.0f, 5.0f));
	...
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-20.0f, 1.2f, -5.0f));
	...
	rt3d::setMaterial(shaderProgram, material1);
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-20.0f, 1.2f, -8.0f));
	mvStack.top() = glm::rotate(mvStack.top(), float(90.0f*DEG_TO_RADIAN), glm::vec3(-1.0f, 0.0f, 0.0f));
	mvStack.top() = glm::scale(mvStack.top(), glm::vec3(scale*0.05, scale*0.05, scale*0.05));
	rt3d::setUniformMatrix4fv(shaderProgram, "modelview", glm::value_ptr(mvStack.top()));
	rt3d::drawMesh(meshObjects[1], md2VertCount, GL_TRIANGLES);
	...
	rt3d::setUniformMatrix4fv(texturedProgram, "projection", glm::value_ptr(projection));
	glDepthMask(GL_FALSE); // make sure writing to update depth test is off
	...
	rt3d::setUniformMatrix4fv(texturedProgram, "modelview", glm::value_ptr(mvStack.top()));
	rt3d::drawIndexedMesh(meshObjects[0], meshIndexCount, GL_TRIANGLES);
	mvStack.pop();
